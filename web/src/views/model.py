...
def model__edit_GET(id):
    ...
    if id:
        ...
        if result.get("status") != 200:
            return redirect('/picture-set?status=model_not_found')
        #endif
...
def model__delete_GET(id):
    ...
    if result.get("status") != 200:
        return redirect('/picture-set?status=model_delete_failed')
    #endif
    return redirect('/picture-set?status=model_delete_ok')
#enddef
